...
template <typename EventType, typename... EventDataTypes> struct EventBus {
    ...
    auto add(Args &&... args) -> decltype(std::get<e>(events).back()) {
        std::get<e>(this->nextEvents).emplace_back(args...);
        return std::get<e>(this->nextEvents).back();
    }

    template <size_t e>
    void listen(typename std::remove_reference<
                decltype(std::get<e>(callbacks).back())>::type callback) {
        std::get<e>(this->callbacks).push_back(callback);
    }

    void clear() { this->_clearAll<0, EventDataTypes...>(EventDataTypes()...); }

    void step() { this->_stepAll<0, EventDataTypes...>(EventDataTypes()...); }
...
private:
    ...
    template <int i, typename Head, typename... Tail>
    void _stepAll(Head head, Tail... tail) {
