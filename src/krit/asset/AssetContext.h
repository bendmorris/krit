...
typedef std::unordered_map<std::string, std::shared_ptr<void>> AssetMap;
...
struct AssetContext {
    ...
    std::shared_ptr<void> get(const std::string &type, const std::string &id) {
        ...
        if (foundAsset == map.end()) {
            ...
            map.insert(std::make_pair(id, result));
            return result;
        } else {
            // found; return
    ...
    std::shared_ptr<std::string> getText(const std::string &id) {
        return std::static_pointer_cast<std::string>(this->get("txt", id));
    }

    std::shared_ptr<ImageData> getImage(const std::string &id) {
        return std::static_pointer_cast<ImageData>(this->get("img", id));
    }

    std::shared_ptr<BitmapFont> getBitmapFont(const std::string &id) {
        return std::static_pointer_cast<BitmapFont>(this->get("bmf", id));
    }

    std::shared_ptr<TextureAtlas> getTextureAtlas(const std::string &id) {
        return std::static_pointer_cast<TextureAtlas>(this->get("tex", id));
    }
};
