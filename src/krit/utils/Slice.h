...
struct StringSlice: public Slice<char> {
    ...
    template <typename Arg> bool operator==(Arg other) { return this->cmp(other); }

    int cmp(char *other) {
        return strncmp(this->data, other, this->length);
    }
    ...
    int cmp(StringSlice &other) {
        return this->length == other.length && strncmp(this->data, other.data, this->length);
    }
};
