...
template <typename ModelType> struct DataModel {
    ...
    ModelType &operator[](const std::string &id) {
    ...
    static std::pair<std::string, std::string> parseDelimited(const std::string &value, char delimiter) {
    ...
    static void applyToMap(yaml_document_t *doc, yaml_node_t *node, std::function<void(std::string&, yaml_node_t*)> fn) {
        for (yaml_node_pair_t *it = node->data.mapping.pairs.start; it < node->data.mapping.pairs.top; ++it) {
    ...
    static void applyToSequence(yaml_document_t *doc, yaml_node_t *node, std::function<void(yaml_node_t*)> fn) {
        for (yaml_node_item_t *childIndex = node->data.sequence.items.start; childIndex < node->data.sequence.items.top; ++childIndex) {
    ...
    void parse(const char *path) {
        ...
        for (yaml_node_item_t *it = root->data.sequence.items.start; it < root->data.sequence.items.top; ++it) {
    ...
    void finish(ModelType &item) {
        this->idMap[item.id] = item.index;
    }
