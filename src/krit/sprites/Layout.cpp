...
unordered_map<string, string> &collectAttrs(const char **attrs) {
    static unordered_map<string, string> _attrMap;
    _attrMap.clear();
...
void LayoutRoot::parseAndApplyStyle(unordered_map<string, string> &attrMap, shared_ptr<VisibleSprite> e) {
...
ImageRegion LayoutRoot::parseSrc(unordered_map<string, string> &attrMap, AssetContext *asset) {
    ...
    if (found != attrMap.end()) {
        shared_ptr<TextureAtlas> atlas = asset->getTextureAtlas(found->second);
        return atlas->getRegion(attrMap["region"]);
    }
}

void parseBackdrop(LayoutParseData *data, unordered_map<string, string> &attrMap) {
    LayoutRoot *root = data->root;
    shared_ptr<LayoutNode> node = data->node;
    ImageRegion src = LayoutRoot::parseSrc(attrMap, data->asset);
    shared_ptr<Backdrop> backdrop = make_shared<Backdrop>(src);
...
void parseImg(LayoutParseData *data, unordered_map<string, string> &attrMap) {
    LayoutRoot *root = data->root;
    shared_ptr<LayoutNode> node = data->node;
    ImageRegion src = LayoutRoot::parseSrc(attrMap, data->asset);
    shared_ptr<Image> img = make_shared<Image>(src);
    LayoutRoot::parseAndApplyStyle(attrMap, img);
    node->attachSprite(img);
}

void parseLabel(LayoutParseData *data, unordered_map<string, string> &attrMap) {
    LayoutRoot *root = data->root;
    shared_ptr<LayoutNode> node = data->node;

    // parse text options first
    shared_ptr<BitmapFont> font = data->asset->getBitmapFont(attrMap["font"]);
    ...
    shared_ptr<BitmapText> txt = make_shared<BitmapText>(options);
...
void parseSpine(LayoutParseData *data, unordered_map<string, string> &attrMap) {
    LayoutRoot *root = data->root;
    shared_ptr<LayoutNode> node = data->node;

    // parse skeleton name
    shared_ptr<SpineSprite> spine = make_shared<SpineSprite>(*data->asset, attrMap["skeleton"]);
...
void parseNineSlice(LayoutParseData *data, unordered_map<string, string> &attrMap) {
    LayoutRoot *root = data->root;
    shared_ptr<LayoutNode> node = data->node;
    ImageRegion src = LayoutRoot::parseSrc(attrMap, data->asset);
    int lw, rw, th, bh;
    ...
    shared_ptr<NineSlice> img = make_shared<NineSlice>(src, lw, rw, th, bh);
    LayoutRoot::parseAndApplyStyle(attrMap, img);
    node->attachSprite(img);
}

void parseButton(LayoutParseData *data, unordered_map<string, string> &attrMap) {
    LayoutRoot *root = data->root;
    shared_ptr<LayoutNode> node = data->node;
    ImageRegion src = LayoutRoot::parseSrc(attrMap, data->asset);
    int lw, rw, th, bh;
    ...
    shared_ptr<Button> btn = make_shared<Button>(src, lw, rw, th, bh, options, label);
...
void parseDiv(LayoutParseData *data, unordered_map<string, string> &attrMap) {
    data->node->keepSize = true;
}

void parsePlaceholder(LayoutParseData *data, unordered_map<string, string> &attrMap) {
...
void layoutStartElement(void *userData, const char *name, const char **attrs) {
    ...
    unordered_map<string, string> &attrMap = collectAttrs(attrs);
    shared_ptr<LayoutNode> node = make_shared<LayoutNode>();
...
unordered_map<string, LayoutParseFunction*> LayoutRoot::parsers = {
    {"div", &parseDiv},
