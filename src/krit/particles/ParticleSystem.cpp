...
ParticleSystem::ParticleSystem(const std::string &path) {
    ...
    int length;
    const unsigned char *contents = static_cast<unsigned char *>(
        static_cast<void *>(IoRead::read(path, &length)));
    ...
    for (yaml_node_pair_t *it = root->data.mapping.pairs.start;
        ...
        if (!strcmp(keyName, "functions")) {
            ...
            for (yaml_node_pair_t *it = value->data.mapping.pairs.start;
                ...
                for (yaml_node_item_t *childIndex =
                         value->data.sequence.items.start;
                     childIndex < value->data.sequence.items.top;
                     ++childIndex) {
                    yaml_node_t *child =
                        yaml_document_get_node(&doc, *childIndex);
                    auto &particle = effect->addParticle();
                    assert(child->type == YAML_MAPPING_NODE);
                    for (yaml_node_pair_t *it = child->data.mapping.pairs.start;
                         it < child->data.mapping.pairs.top; ++it) {
                        yaml_node_t *keyNode =
                            yaml_document_get_node(&doc, it->key);
                        char *keyName =
                            yamlStringToCString(keyNode->data.scalar.value);
                        yaml_node_t *value =
                            yaml_document_get_node(&doc, it->value);
                        if (!strcmp(keyName, "atlas")) {
                            particle.atlas =
                                yamlStringToCString(value->data.scalar.value);
                            ...
                            if (!strcmp(blend, "alpha"))
                                particle.blend = Alpha;
                            else if (!strcmp(blend, "add"))
                                particle.blend = Add;
                            else if (!strcmp(blend, "subtract"))
                                particle.blend = Subtract;
                            else if (!strcmp(blend, "multiply"))
                                particle.blend = Multiply;
                            else if (!strcmp(blend, "screen"))
                                particle.blend = BlendScreen;
                            else
                                panic("%s: unrecognized blend mode %s",
                                      path.c_str(), blend);
                            ...
                            particle.aligned = !strcmp(
                                "true",
                                yamlStringToCString(value->data.scalar.value));
                        } else if (!strcmp(keyName, "duration")) {
                            particle.duration = atof(
                                yamlStringToCString(value->data.scalar.value));
                        } else if (!strcmp(keyName, "layer")) {
                            particle.layer = atoi(
                                yamlStringToCString(value->data.scalar.value));
                        } else if (!strcmp(keyName, "props")) {
                            for (yaml_node_pair_t *it =
                                     value->data.mapping.pairs.start;
                                 it < value->data.mapping.pairs.top; ++it) {
                                yaml_node_t *keyNode =
                                    yaml_document_get_node(&doc, it->key);
                                char *keyName = yamlStringToCString(
                                    keyNode->data.scalar.value);
                                yaml_node_t *value =
                                    yaml_document_get_node(&doc, it->value);
#define GET_VAL(i)                                                             \
    atof(yamlStringToCString(                                                  \
        yaml_document_get_node(&doc, *(value->data.sequence.items.start + i))  \
            ->data.scalar.value))
#define PARSE_STATIC(name) (particle.props.set##name(GET_VAL(0), GET_VAL(1)))
#define PARSE_DYNAMIC(name)                                                    \
    (particle.props.set##name(GET_VAL(0), GET_VAL(1), GET_VAL(2), GET_VAL(3)))
                                if (!strcmp(keyName, "originX")) {
                                    PARSE_STATIC(OriginX);
                                } else if (!strcmp(keyName, "originY")) {
                                    PARSE_STATIC(OriginY);
                                } else if (!strcmp(keyName, "duration")) {
                                    PARSE_STATIC(Duration);
                                } else if (!strcmp(keyName, "direction")) {
                                    PARSE_STATIC(Direction);
                                } else if (!strcmp(keyName, "red")) {
                                    PARSE_DYNAMIC(Red);
                                } else if (!strcmp(keyName, "green")) {
                                    PARSE_DYNAMIC(Green);
                                } else if (!strcmp(keyName, "blue")) {
                                    PARSE_DYNAMIC(Blue);
                                } else if (!strcmp(keyName, "alpha")) {
                                    PARSE_DYNAMIC(Alpha);
                                } else if (!strcmp(keyName, "scale")) {
                                    PARSE_DYNAMIC(Scale);
                                } else if (!strcmp(keyName, "rotation")) {
                                    PARSE_DYNAMIC(Rotation);
                                } else if (!strcmp(keyName, "distance")) {
                                    PARSE_DYNAMIC(Distance);
                                } else if (!strcmp(keyName, "orthoDistance")) {
                                    PARSE_DYNAMIC(OrthoDistance);
                                } else {
                                    panic("%s: unexpected particle system YAML "
                                          "prop key: %s",
                                          path.c_str(), keyName);
                                }
                            }
            ...
            panic("%s: unexpected particle system YAML key: %s", path.c_str(),
                  keyName);
        }
    }
