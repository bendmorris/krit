...
ParticleType::ParticleType(int id, ImageRegion region, BlendMode blend)
    : id(id), image(region),
    colorStart(Color::white(), Color::white()),
    colorEnd(Color::white(), Color::white()),
    alphaStart(1, 1),
    alphaEnd(1, 1),
    scaleStart(1, 1),
    scaleEnd(1, 1),
...
void Emitter::emit(int effectType) {
...
void Emitter::emit(ParticleType &type, int count) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        particle.rotation.setTo(
            krit::lerp(type.rotationStart.start, type.rotationStart.end, r(rng)),
            krit::lerp(type.rotationEnd.start, type.rotationEnd.end, r(rng))
        );
        particle.start = this->position;
        particle.duration = krit::lerp(type.duration.start, type.duration.end, r(rng));
        particle.origin.setTo(r(rng) * 2 - 1, r(rng) * 2 - 1);
        particle.origin.normalize(krit::lerp(type.distanceStart.start, type.distanceStart.end, r(rng)));
        ...
        particle.move.multiply(krit::lerp(type.distanceEnd.start, type.distanceEnd.end, r(rng)));
    }
}
...
void Emitter::update(UpdateContext &ctx) {
    ...
    while (i < this->_particles.size()) {
        auto &it = this->_particles[i];
        if (!it.isTrail && it.type->trail > 0) {
            this->_particles.emplace_back(it.type);
            auto &trail = this->_particles.back();
            trail.duration = it.type->trail;
            trail.isTrail = true;
            ...
            c.a *= 0.5;
            trail.color.setTo(c);
            trail.scale.setTo(krit::lerp(it.scale.start, it.scale.end, it.decay));
            trail.angle = it.angle;
            Point m(it.move);
            // m.multiply(maybeLerp(it.decay, it.type->distanceLerp));
            m.multiply(it.decay);
            trail.origin.add(m);
        }
        ...
        if (it.decay >= 1) {
            if (this->_particles.size() > 1) {
                std::iter_swap(this->_particles.begin() + i, this->_particles.end() - 1);
            }
...
void Emitter::render(RenderContext &ctx) {
    for (auto &particle : this->_particles) {
        ...
        if (particle.isTrail) {
            ...
            image.color = particle.color.start.lerp(particle.color.end, maybeLerp(particle.decay, type.colorLerp));
            image.scale.setTo(krit::lerp(particle.scale.start, particle.scale.end, maybeLerp(particle.decay, type.scaleLerp)));
        }
        image.angle = particle.angle + krit::lerp(particle.rotation.start, particle.rotation.end, maybeLerp(particle.decay, type.rotationLerp));
        image.render(ctx);
    }
}
