...
template <typename T, typename Self> struct BasePoint {
    T x = 0, y = 0;
    ...
    Self lerp(Self &other, float mix) {
        if (mix >= 1)
            ...
            return Self(this->x * (1 - mix) + other.x * mix,
...
struct Point : public BasePoint<float, Point> {
    Point() {}
    Point(float x, float y) : BasePoint<float, Point>(x, y) {}
};
struct IntPoint : public BasePoint<int, IntPoint> {
    IntPoint() {}
