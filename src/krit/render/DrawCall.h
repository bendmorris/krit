...
struct DrawCall {
    ...
    void addTriangle(DrawCommandBuffer *buf, const Triangle &t, const Triangle &uv, const Color &color);
    void addTriangle(DrawCommandBuffer *buf, float t1, float t2, float t3, float t4, float t5, float t6, float uv1, float uv2, float uv3, float uv4, float uv5, float uv6, const Color &color);
};
