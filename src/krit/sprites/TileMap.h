...
struct TileMap : public VisibleSprite {
    ...
    Dimensions getSize() override {
        return Dimensions(this->properties.sizeInTiles.width() *
                              this->properties.tileSize.width(),
                          this->properties.sizeInTiles.height() *
                              this->properties.tileSize.height());
    }

    int16_t &getTile(int x, int y) {
        return this->tileData[y * this->properties.sizeInTiles.width() + x];
    }
...
private:
    ...
    void _init() {
        ...
        this->clip.setTo(0, 0, this->properties.sizeInTiles.width(),
                         this->properties.sizeInTiles.height());
    }
};
