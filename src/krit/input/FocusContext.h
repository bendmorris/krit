...
struct FocusContext {
    ...
    void add(UpdateContext *ctx, VisibleSprite *sprite,
             SelfUpdateSignal onGotFocus = nullptr,
             SelfUpdateSignal onLostFocus = nullptr) {
        bool wasEmpty = this->registeredObjects.empty();
        this->registeredObjects.emplace_back(
            sprite, FocusCallbacks(onGotFocus, onLostFocus));
        if (wasEmpty) {
            this->focus = 0;
    ...
    void remove(VisibleSprite *sprite) {
        for (auto it = this->registeredObjects.begin();
             it != this->registeredObjects.end(); ++it) {
            if (it->first == sprite) {
    ...
    void clear() { this->registeredObjects.clear(); }

    VisibleSprite *focused() {
        return this->registeredObjects.empty() ? nullptr
                                               : registeredObjects[focus].first;
    }
